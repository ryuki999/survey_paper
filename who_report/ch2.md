...
* ウイルスの進化を論じる際には、変異率と進化率（または置換率）を区別することが極めて重要である。変異率とは、親ウイルスから子ウイルスへのRNAのコピーの際に発生するエラーの数を考慮した指標であり、通常、複製ごとのゲノムあたりの変異数で測定される。変異率は、実験室で既知の回数の複製の前後で遺伝子の多様性を測定するために、ウイルス集団全体の配列を決定するなど、いくつかの方法で実験的に推定することができる。ほとんどの変異は有害であり、そのような変異を持つ個々のウイルスはしばしば複製できない。
...
* RNAウイルスは一般的に進化速度が速く、多くのウイルスは数日から数週間ごとに遺伝子が変化しています。そのため、一部のRNAウイルスは、宿主間の感染とほぼ同じ時間スケールで遺伝子の置換を獲得する。SARS-CoV-2の場合、ヒト間の感染の発生率は、感染したウイルス系統が遺伝子置換を獲得する速度よりも平均的に高い。SARS-CoV-2の系統は、数日ではなく数週間から数カ月かけて遺伝的多様性を獲得していくため、感染連鎖の中で直接隣り合った患者が、同一のゲノムを持つウイルスに感染している可能性がある。流行期におけるウイルスのゲノム多様性の蓄積パターンを分析することで、疫学的プロセスに関する推論を行うことができる。これは、Grenfellらが造語した「hylodynamics」という包括的な用語の下で行われている一連の系統学的手法の焦点である。

* 系統力学的手法は、確認された症例に基づく他の疫学的分析を補完・補強することができるため、流行の調査に有用である。
 * 第一に、いくつかのhylodynamic法は、調査活動の時間的変化や症例の検出のばらつきなど、診断調査におけるバイアスの影響を受けにくい、あるいは異なる影響を受ける可能性がある。
 * 第二に、hylodynamicsは、サンプリングの時間枠外（例えば、最初の症例が確認される前）に発生する流行の特徴を明らかにすることができる。
 * 第三に、hylodynamic分析によって、特定の異なるウイルス系統の集団動態を直接知ることができる。

* 系統樹解析では、確率論的モデルを用いて、サンプリングしたゲノムの系統樹と対象となる疫学的パラメータを結びつける。そのため、どの配列が一緒に集まっているかだけでなく、サンプルされたウイルスゲノムのサンプルされていない最新の共通祖先（MRCA）がいつ存在したかという情報を含む年代別の系統樹を推論する必要があります。配列決定されたサンプルから得られたウイルスについては、サンプリング日がわかっているが（すなわち、木の先端）、MRCA（すなわち、内部ノード）は系統発生的に推測されるため、その存在時期を推定する必要がある。これらの時間を推定するには、クロックレート（系統樹の枝に沿った遺伝子置換の平均速度）をパラメータとした分子時計モデルを使用する必要がある。
